...
def leer(grupo_gal,GR):
    ...
    path='Gal_%s.galfit.01.band'%(grupo_gal) #lee los archivos .band
...
def grafico(grupo_gal):
	f = fits.open('Gal_%s.fits'%(grupo_gal))
	...
	plt.savefig('Gal_%s.svg'%(grupo_gal),format='svg', dpi=1200)
	#plt.show()
	plt.close()
	return

Fi=Table.read('GalfitM_output_malos_A1644.csv')
...
for i in range(len(Fi)):
	print(i)
	Tabla_gal=S[(S["ID"]==Fi["ID"][i]) ]
	print(Tabla_gal)
	galaxy=Fi['Grupo_Gal'][i]
	if galaxy=='SPLUS-n12s32_1175_1375_0':
		hola=1
	elif galaxy=='SPLUS-n12s32_1375_1375_0':
		hola=1
	elif galaxy=='SPLUS-n12s32_1375_1375_1':
		hola=1
	elif galaxy=='SPLUS-n12s32_1375_1375_2':
		hola=1
	elif galaxy=='SPLUS-n12s32_1375_1375_3':
		hola=1
	elif galaxy=='SPLUS-n13s31_825_9625_1':
		hola=1
	else:
		grafico(galaxy)
		#T=leer(galaxy,Tabla_gal)
		#if float(T[1])<3:
		#		Tabla.append(T[0])
		#		ch.append(T[1])
		#		GN.append(galaxy)
...
ascii.write(Table_GalfitM, 'GalfitM_output_A1644_malos_corre.csv', format='csv', fast_writer=False,overwrite=True) #guarda la tabla completa con los resultados de galfitm y splus
Chi_Table= Table([GN,ch],names=('Group_Gal','Chi^2/nu'))
ascii.write(Chi_Table, 'Chi2_A1644_malos_corre.csv', format='csv', fast_writer=False,overwrite=True) #guarda el chi cuadrado del ajuste para cada grupo
...
ascii.write(Table_GalfitM2, 'GalfitM_output_malos_A1644_malos.csv', format='csv', fast_writer=False,overwrite=True) #guarda la tabla completa con los resultados de galfitm y splus
Chi_Table2= Table([GN2,ch2],names=('Group_Gal','Chi^2/nu'))
ascii.write(Chi_Table2, 'Chi2_malos_A1644_malos.csv', format='csv', fast_writer=False,overwrite=True) #guarda el chi cuadrado del ajuste para cada grupo
"""
